...
def create_maze_btn():
    ...
    maze_btn_w = 120
    maze_btn_h = 30
    maze_btn_text = font_small.render("Create Maze", True, white)

    pygame.draw.rect(screen, pygame.Color(235, 20, 20), (maze_btn_pos[0], maze_btn_pos[1], maze_btn_w, maze_btn_h), width = 0, border_radius = 8)
...
def create_maze():
    ...
    for i in range(rows):
        for j in range(cols):
            if maze[i][j] == 1:
                pygame.draw.rect(screen, pygame.Color(40,40,40), (box_node_array[i][j].xdist_position - 0.5, box_node_array[i][j].ydist_position - 0.5, 11, 11))
    if maze[56][120] == 1:
...
def run_a_star_algorithm(allow_diagonal_movement = False):
    ...
    while len(open_list) > 0:
        ...
        if current_node.position is not None:
            if current_node.position[0] == end_index[0] and current_node.position[1] == end_index[1]:
...
def no_path_found():
    ...
    screen.blit(no_path_text, (880, 75))
